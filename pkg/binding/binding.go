...
func Json(jsonStruct interface{}, ifacePtr ...interface{}) macaron.Handler {
	return func(ctx *macaron.Context) {
		...
		// 从url参数取值，如?Name=Test
		ctx.Req.ParseForm()
		for k, v := range ctx.Req.Form { // according param(k) to get and set field. first char to up,
			k = strFirstToUpper(k)
			field := val.FieldByName(k)
			setWithProperType(field.Kind(), v[0], field, k, errors)
		}

		// 从url路径取值，如/project/111
		for k, v := range ctx.GetParams() {
			field := val.FieldByName(k[1:])
			setWithProperType(field.Kind(), v, field, k[1:], errors)
		}
